...
export default function Footer({ navLinks }) {
  return (
    <footer className="relative z-10 bg-zinc-950 text-white dark:bg-zinc-950 py-12 px-6 border-t border-zinc-800 select-none shrink-0 mt-auto">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row items-start justify-between gap-8">
        ...
        <div className="space-y-4 max-w-md">
          <Link to="/" className="flex items-center gap-2 group shrink-0">
            <LogoBug className="h-8 w-8 text-white transition-all duration-200 group-hover:scale-105 shrink-0" />
        ...
        {/* Right Column: Navigation Links and Social Icons */}
        <div className="flex flex-col items-start md:items-end gap-6">
            ...
            {navLinks.map((link) => {
              return (
                <Link
                  key={link.label}
                  to={link.path}
                  className="transition-colors hover:text-rose-500 text-zinc-400 text-sm font-semibold"
                >
                  {link.label}
                </Link>
              )
          ...
          <div className="flex items-center gap-6">
            ...
            <a href="https://x.com" target="_blank" rel="noopener noreferrer" className="text-zinc-400 hover:text-white transition-colors" aria-label="X (Twitter)">
              <svg className="h-4 w-4 fill-current" viewBox="0 0 24 24">
                <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
              </svg>
            </a>
            <a href="https://linkedin.com" target="_blank" rel="noopener noreferrer" className="text-zinc-400 hover:text-white transition-colors" aria-label="LinkedIn">
              <svg className="h-5 w-5 fill-current" viewBox="0 0 24 24">
